  ...
  router.get("/chat/sessions/:id", async (req, res, next) => {
    try {
      ...
      const messages = await db.query(
        "SELECT id, role, content, citations, grounded, synthesis, fell_back AS \"fellBack\", grounding_score AS \"groundingScore\", created_at FROM chat_messages " +
  ...
  router.post("/chat/sessions/:id/messages", async (req, res, next) => {
    try {
      ...
      const assistant = await db.query(
        "INSERT INTO chat_messages (session_id, role, content, citations, grounded, synthesis, fell_back, grounding_score) " +
          "VALUES ($1, 'assistant', $2, $3::jsonb, $4, $5, $6, $7::real) RETURNING id, role, content, citations, grounded, synthesis, fell_back, grounding_score AS \"groundingScore\", created_at",
        [
          req.params.id,
          rag.answer,
          JSON.stringify(rag.citations ?? []),
          rag.grounded ?? true,
          (rag as { synthesis?: string }).synthesis ?? null,
          (rag as { fellBack?: boolean }).fellBack ?? false,
          (rag as { topScore?: number }).topScore ?? null,
        ],
      );
